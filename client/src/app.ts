...
interface IThuoc_Tinh {
    ...
    luu_y: string[];
    image: string[];
    kich_thuoc_trong_kinh: string;
    cau_kinh: string;
    chieu_dai_cang_kinh: string;
}
...
class CSan_Pham implements ISan_Pham {
    ...
    constructor(
        id: number, name: string, price: number, salePrice: number, discountPercentage: number,
        img: string, description: string, quantity: number, code: string, width: number,
        lengthmm: number, glasses: string, color: string, category: number, dateAdded: string
    ) {
        this.id = id;
        this.name = name;
        this.price = price;
...
// Lớp CMatKinh kế thừa từ CSan_Pham và implements IThuoc_Tinh
class CMatKinh extends CSan_Pham implements IThuoc_Tinh {
    ...
    constructor(
        ...
        img: string, description: string, quantity: number, code: string, width: number,
        lengthmm: number, glasses: string, color: string, category: number, dateAdded: string,
        image: string[], chat_lieu: string, kich_thuoc: string, 
        mo_ta: string, dac_diem_noi_bat: string, ung_dung: string, luu_y: string[],
        kich_thuoc_trong_kinh: string, cau_kinh: string, chieu_dai_cang_kinh: string
    ) {
        ...
        // Gán giá trị cho các thuộc tính của IThuoc_Tinh và các thuộc tính thêm vào
        this.image = image;
...
export const layspmoi = async(sosp: number = 10) =>{
...
export const layspkm = async (sospkm: number = 8) => {
    ...
    let str = ``;
    data.forEach(spkm => {
        // Chỉ xử lý sản phẩm nếu `salePrice` có giá trị hợp lệ
...
export const motspkm = (spkm) => {
    let {
        id, name, price, salePrice, discountPercentage, img, description,
        quantity, code, width, lenght, glasser, color, category, dateAdded
    } = spkm;

    let showspkm: CSan_Pham = new CSan_Pham(
        id, name, price, salePrice, discountPercentage, img,
        description, quantity, code, width, lenght, glasser,
        color, category, dateAdded
    );

    // Kiểm tra nếu `salePrice` hợp lệ
...
export const layallsp = async (soallsp: number = 9) => {
    console.log("Limit:", soallsp);  // log số lượng sản phẩm
...
export const Lay_all_san_pham = (allsp) => {
    const showallsp = new CSan_Pham(
        allsp.id, allsp.name, allsp.price, allsp.salePrice, allsp.discountPercentage,
        allsp.img, allsp.description, allsp.quantity, allsp.code, allsp.width,
        allsp.lengthmm, allsp.glasses, allsp.color, allsp.category, allsp.dateAdded
    );

    const isFullDiscount = parseFloat(showallsp.Phantramgiam()) === 100;  // use method on the instance of CSan_Pham
...
// lấy sp theo dm
export const lay_sp_or_dm = async (id_danhmuc: string, sosp: number = 6) =>{
...
export const lay1sp = async (id: number = 0) => {
  // Lấy dữ liệu sản phẩm và thuộc tính từ server
  ...
  // Destructuring dữ liệu sản phẩm và thuộc tính
  ...
  // Nếu image là mảng, sử dụng nó để tạo các ảnh thu nhỏ
  ...
  // HTML cho chi tiết sản phẩm với nội dung động, bao gồm hình ảnh và các tùy chọn
  let str = `
  ...
  `;
  // Phần mô tả sản phẩm và thông tin bổ sung
  str += `
  <div class="mota">
  ...
  // Phần đánh giá sản phẩm với form nhập đánh giá
  str += `
  <br>
